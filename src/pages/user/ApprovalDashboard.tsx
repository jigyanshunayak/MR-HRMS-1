...
export default function ApprovalDashboard() {
  ...
  const fetchPendingApprovals = async () => {
    try {
      ...
      console.log("Fetching pending approvals with token:", token);
  ...
  const handleQuickApprove = async (approvalId: string, employeeName: string) => {
    console.log("Approving employee:", { approvalId, employeeName });
    ...
    if (result.isConfirmed) {
      try {
        ...
        console.log("Sending approve request:", { approvalId, token });
        ...
        Swal.fire(
          "Error",
          error.response?.data?.error || "Failed to approve",
          "error"
        );
      }
    }
  ...
  const handleReject = async (approvalId: string, employeeName: string) => {
    console.log("Rejecting employee:", { approvalId, employeeName });
    ...
    if (reason) {
      try {
        ...
        console.log("Sending reject request:", { approvalId, reason, token });
  ...
  const canTakeAction = (approvalType: string) => {
    ...
    const allowed = roleMapping[approvalType]?.includes(userRole);
                  ...
                  <div className="flex flex-col space-y-2 ml-4">
                    {canTakeAction(approval.approval_type) && (
                      <>
                        ...
                        <button
                          onClick={() =>
                            handleReject(approval.id, approval.employee.name)
                          }
                    ...
                    <button
                      onClick={() =>
                        window.open(
                          `/employee/${approval.empid}/joining-kit`,
                          "_blank"
                        )
                      }
            ...
            <div className="mt-8 grid grid-cols-1 md:grid-cols-3 gap-6">
              ...
              <div className="bg-white p-4 rounded-lg shadow border">
                <div className="text-2xl font-bold text-yellow-600">
                  {
                    pendingApprovals.filter((a) => a.approval_type === "hr")
                      .length
                  }
                </div>
                ...
                <div className="text-2xl font-bold text-purple-600">
                  {
                    pendingApprovals.filter(
                      (a) => a.approval_type === "branch_manager"
                    ).length
                  }
                </div>
